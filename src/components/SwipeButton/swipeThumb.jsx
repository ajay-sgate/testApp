...
const SwipeThumb = props => {
  ...
  const animatedWidth = useRef(new Animated.Value(defaultContainerWidth)).current;
  ...
  function setBackgroundColors() {
    const { railFillBackgroundColor } = props;
    ...
    return (
      <View style={[styles.icon, { ...dynamicStyles }]}>
        {!ThumbIconComponent && thumbIconImageSource && (
          <Image resizeMode='contain' style={{ height: 14, width: 10 }} source={thumbIconImageSource} />
        )}
        {ThumbIconComponent && (
          <View>
            <ThumbIconComponent />
  ...
  return (
    <>
      {screenReaderEnabled ? (
        ...
        <Animated.View
          style={[panStyle, { borderWidth: 0, backgroundColor: status? Colors.green: Colors.red, justifyContent:'center', }]}
          {...panResponder.panHandlers}
          pointerEvents={shouldDisableTouch ? 'none' : 'auto'}>
         
          <Text
              style={[styles.endTitle,{opacity:defaultWidth- 130}]}>
              {'Release'}
          </Text>
          {renderThumbIcon()}
